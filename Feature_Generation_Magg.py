...
def get_magg_features(experiment_tuple, agg_mins = agg_mins):
    
    day,device = experiment_tuple
    file_name = 'data/split-trace/day_'+str(day)+'_'+str(device)+'.csv'
    df = pd.read_csv(file_name)
    df['packet_polarity'] = df['ip.src'].apply(check_polarity)
    ...
    for time in range(96):
        ...
        if temp.shape[0]>0:
            ...
            out_udp_flow_duration,int(actual_id[device])]
...
def collate_agg_traces():    
    ...
    if len(experiments_all_local) < 20:
        return None
